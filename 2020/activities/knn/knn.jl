...
md"
supervised learning: data is labeled/associated with a known outcome. task is to predict the outcome.
unsupervised learning: data is unlabeled. task is to find patterns or structure in the data.
"
...
md"
both under the category of supervised learning. classifiation, the output is qualitative. regression, the output is quantitative.
"
...
# ╔═╡ 98e86b6a-2573-11eb-392b-a394df957fa5
md"
!!! note
...
begin
	...
	# load in the data
	df = CSV.read("wdbc.data", DataFrame, header=header)
...
outcome_to_color = Dict("M" => "red", "B" => "green")
...
ids_train, ids_test = train_test_split(1:nrow(df), test_size=0.3, shuffle=true)

# ╔═╡ 8b713356-255d-11eb-2df9-41b1d0627975
df_train, df_test = df[ids_train, :], df[ids_test, :]
...
describe(df_train, :min, :max, :mean)
...
begin
	for feature in [:max_smoothness, :mean_texture]
		# compute min, max of training data
		f_min = minimum(df_train[:, feature])
		f_max = maximum(df_train[:, feature])
		f_range = f_max - f_min
		
		# new col name for normalized feature
		s_feature_col_name = Symbol("s_" * String(feature))
		
		for data in [df_test, df_train]
			data[!, s_feature_col_name] = (data[:, feature] .- f_min) / f_range
		end
	end
...
begin
	...
	for df_c in groupby(df_train, :outcome)
	    outcome = df_c[1, :outcome]
	    scatter(df_c[:, :s_max_smoothness], df_c[:, :s_mean_texture], 
			    label="$outcome", marker="o", facecolor="None", 
			    edgecolor=outcome_to_color[outcome])
	end
	legend()
...
function df_to_X_y(data::DataFrame)
	n_tumors = nrow(data)
	
	X = zeros(n_tumors, 2)
	y = zeros(n_tumors)
	
	for (i, tumor) in enumerate(eachrow(data))
		X[i, 1] = tumor[:s_max_smoothness]
		X[i, 2] = tumor[:s_mean_texture]
		y[i] = tumor[:outcome] == "B" ? 0 : 1
	end
...
function fit_kNN_model(k::Int, X_train::Array{Float64, 2}, y_train::Array{Float64, 1})
...
knn_2 = fit_kNN_model(2, X_train, y_train)
...
begin
	...
	scatter([X_new[1]], [X_new[2]], label="new point", color="blue")
	legend()
	gca().set_aspect("equal")
	gcf()
end
...
ŷ_test = knn_2.predict(X_test)

# ╔═╡ 0f657bda-256e-11eb-3d6b-97493d220908
test_acc = sum(ŷ_test .== y_test) / length(y_test)
...
begin
	plot_confusion_matrix(knn_2, X_test, y_test, 
						  display_labels=["benign", "malignant"], cmap="Greens")
	gcf()
end
...
begin
	...
	for (i, k) in enumerate(ks)
	    knn = fit_kNN_model(k, X_train, y_train)
	...
	axvline(x=argmax(test_accuracy), color="k", linestyle="--")
	legend()
	xlabel(L"$k$")
	ylabel("accuracy")
	gcf()
...
k_opt = argmax(test_accuracy)

# ╔═╡ 4575a944-2570-11eb-1362-afda09b198e1
draw_decision_boundary(fit_kNN_model(k_opt, X_train, y_train))
